-- Active: 1731619206034@@127.0.0.1@3306@ordenesSQL
...
/*------------------------------------------------------------------------------------------------------------*/
--FormulsOrden view--
DROP VIEW vw_PRODORD_formulsOrden
...
CREATE VIEW vw_PRODORD_formulsOrden as
    ...
    select * from mat_prima where orden = 9

SELECT *
FROM vw_PRODORD_formulsOrden
WHERE noOrden = 6

/* -------------------------------MATERIA PRIMA ING----------------------------------------------------------- */
DROP VIEW vw_PRODORD_formuMatPrima
...
CREATE VIEW vw_PRODORD_formuMatPrima as
    ...
    SELECT * FROM vw_PRODORD_formuMatPrima
WHERE noOrden = 9

/*------------------------------------------------------------------------------------------------------------*/
--FormulsActiveOrden view--

DROP VIEW vw_PRODORD_formulsActive
...
SELECT DISTINCT
    ...
    select * from vw_PRODORD_formulsActive
    WHERE noOrden = 7

    
/* -------------------------------MATERIA PRIMA ING----------------------------------------------------------- */
DROP VIEW vw_PRODORD_formuMatPrimaAct
...
CREATE VIEW vw_PRODORD_formuMatPrimaAct as
    ...
    SELECT * FROM vw_PRODORD_formuMatPrimaAct
WHERE noOrden = 2


/*------------------------------------------------------------------------------------------------------------*/
--EmpaqueOrden view--
DROP VIEW vw_PRODORD_empaqueOrden
...
CREATE VIEW vw_PRODORD_empaqueOrden as
    ...
    select * from vw_PRODORD_empaqueOrden
WHERE noOrden = 2 and codProd = 2

/* ---------------------------------------VISTAS SUPLEMENTOS ALIMENTICIOS---------------------------------------- */

DROP VIEW vw_PRODORD_info_suple
...
select * from vw_PRODORD_info_suple
    where noOrden = 7
...
select tipo_producto from producto where codigo = 2

select* from vw_info_Prod where codProd = 2

select f.ingredientes,
i.descripcion, f.cantidadIng
from formula as f inner join ingredientes as i on f.ingredientes = i.codigo
 where producto  = 2
...
select fa.ingr_activo,
ia.nomIngrediente, fa.cantidadAct
 from form_act as fa 
inner join ingr_activo as ia on fa.ingr_activo = ia.codigo
 where producto = 2
...
ia.nomIngrediente
 from form_act as fa 
inner join ingr_activo as ia on fa.ingr_activo = ia.codigo
 where fa.ingr_activo != 'AMOXI'

where p.codigo = 7

SELECT p.tipo_presentacion, concat(tp.presentacion, ' ', tp.nombre) from producto as p 
        inner join tipo_presentacion as tp on p.tipo_presentacion = tp.codigo
        where p.codigo = 


select * from emp_producto 

select * from mat_empaque


SELECT ep.mat_empaque as matEmp, 
me.nombre as nomEmp, 
ep.cantMatxProd as cant from emp_producto as ep 
...
SELECT codigo,
nomIngrediente
FROM ingr_activo AS ia 
WHERE codigo != 'AMOXI'


UPDATE formula set ingredientes = 'AMOXI', cantidadIng = '0.7', uniMed = 'ml' WHERE producto = 2 AND ingredientes = 'ACICL'
...
DROP TRIGGER calcularMatEmpaque

---- TIENE DETALLES DE LLAVE FORANEA, REVISAR
DELIMITER $$
...
DROP TRIGGER calcularPesoFinalFormula
...
DROP TRIGGER calcularPesoFinalBlend
